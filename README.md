...
Run a [Logos Blockchain](https://github.com/logos-blockchain/logos-blockchain) testnet node on any Ubuntu 24.04 or Debian VPS (Hetzner, Netcup, DigitalOcean, etc.) using Docker Compose.
...
> **⚠️ Security notice:** API keys and dashboard passwords are transmitted in cleartext over HTTP. For production or mainnet use, enable HTTPS via Caddy (see below) or use a reverse proxy with TLS. API keys are stored in plaintext on disk. This setup is appropriate for testnet nodes; do not use it to protect accounts with real value.
...
Username is always `admin`. Leave it empty for public access (fine for a testnet node).

---

## Updating

The easiest way to update is from the **Settings** tab in the dashboard — the **Software Update** card shows your current repo version alongside the latest repo version and lets you trigger an update with one click.

## Breaking upgrade note: upstream node `0.1.2`

The upstream `logos-blockchain/logos-blockchain` release `0.1.2` published on **2026-04-13** is marked as a **breaking** network reset. If you already have an older node volume/config, an in-place rebuild is not enough.

Before switching an existing node to `NODE_VERSION=0.1.2`, back up anything you need, then remove:

```bash
docker compose down -v
```

That clears the named `logos-data` volume, including the old `user_config.yaml` and chain state generated by previous releases. After that, recreate `.env` bootstrap peers if needed and start again with:
...
2. Visit the faucet at **https://testnet.blockchain.logos.co/web/faucet/**
